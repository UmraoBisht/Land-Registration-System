...
import "./1_LandRegistry.sol";

contract TransferOwnerShip {
    Property private propertiesContract;
    LandRegistry private LandRegistryContract;

// _landRegistryContractAddress
    constructor() {
        // Creating Land Registry Object
        LandRegistryContract = new LandRegistry();

        address propertiesContractAddress = LandRegistryContract
    ...
    function addPropertyOnSale(uint256 _propertyId, uint256 _price) public {
        require(
            msg.sender ==
                propertiesContract.getLandDetailsAsStruct(_propertyId).owner,
    ...
    function getRequestedUsers(uint256 saleId)
        public
        view
        returns (RequestedUser[] memory)
    {
        return requestedUsers[saleId];
    }

    // function to get all my requested sales to purchase of a buyer
    function getRequestedSales(address _owner)
        public
        view
    ...
    function getStatusOfPurchaseRequest(uint256 _saleId)
        public
        view
        returns (RequestedUser memory)
    {
    ...
    function getSalesByLocation(uint256 locationId)
        public
        view
        returns (Sales[] memory)
    {
    ...
    function sendPurchaseRequest(uint256 _saleId, uint256 _priceOffered)
        public
    {
        ...
        require(
            ...
            "Property Not in Active State to Purchase"
        );
    ...
    function acceptBuyerRequest(
        ...
        require(
            requestedUsers[sale.saleId].length > 0,
    ...
    function cancelSaleBySeller(uint256 _saleId) public returns (bool) {
        ...
        require(
            sale.state != SaleState.AcceptedToABuyer,
            "Accepted To Buyer,Can't Cancel Sale.Please Cancel Acceptanc First"
        );
        ...
        propertiesContract.changeStateBackToVerified(
            sale.propertyId,
            msg.sender
        );
    ...
    function reactivateSale(uint256 _saleId) public {
        ...
        require(
            sale.owner == msg.sender,
            "Only property owner can Re-activate"
        );
        require(
            sale.state != SaleState.Active,
            "Sale is Already in Active State"
        );
        require(
            sale.state != SaleState.CancelSaleBySeller,
            "Closed Sale can't be reactivated.Please Create New Sale"
        );
        require(
            sale.state != SaleState.AcceptedToABuyer,
            "Closed Sale can't be reactivated.Please Create New Sale"
        ...
        require(
            sale.state != SaleState.Success,
            "Successed Sale can't be reactivated."
        );
    ...
    function rejectingAcceptanceRequestByBuyer(uint256 _saleId) public {
        ...
        require(
            ...
            "Sale state does not allow cancellation"
        );

        // Check that the sale was accepted by the caller
        require(
            sale.acceptedFor == msg.sender,
            "Not Authorized to Reject Acceptance Request of Seller"
        );
    ...
    function rejectingAcceptanceRequestBySeller(uint256 _saleId) public {
        ...
        require(
            sale.owner == msg.sender,
            "Not Authorized to Cancel Acceptance Request of Buyer"
        );
    ...
    function rejectPurchaseRequestOfBuyer(uint256 _saleId, address _buyer)
        public
    {
        Sales storage sale = sales[_saleId];
        ...
        require(
            sale.state != SaleState.CancelSaleBySeller,
            "Can't do operation on Canceled sale"
        );
        require(
            sale.state != SaleState.Success,
            "Can't do operation on Closed Sale"
        );
    ...
    function rerequestPurchaseRequest(uint256 _saleId, uint256 _priceOffered)
        public
    {
        // Get the sales details
        Sales storage sale = sales[_saleId];
        ...
        requestedUsers[sale.saleId][i].state = RequestedUserToASaleState
            .ReRequestedPurchaseRequest;
        requestedUsers[sale.saleId][i].priceOffered = convertToWei(
            _priceOffered
        );
    ...
    function transferOwnerShip(uint256 saleId) public payable {
        ...
        require(
            msg.sender == sale.acceptedFor,
            "Only accepted buyer can complete the sale"
        );

        require(
            msg.value == sale.acceptedPrice,
            "Payment amount must be equal to accepted price"
        );

        require(
            block.timestamp <= sale.deadlineForPayment,
        ...
        for (i = 0; i < propertiesOnSale.length; i++) {
            if (propertiesOnSale[i] == sale.saleId) {
                propertiesOnSale[i] = propertiesOnSale[
                    propertiesOnSale.length - 1
                ];
